...
\item La clausura transitiva de \( \bs{R} \) es la relación más pequeña que extiende a \( \bs{R} \) para que sea transitiva y
...
Ya que las nociones de reducción son conjuntos es posible tomar la unión de dos nociones de reducción para formar otra. Por ejemplo, la noción de reducción \( \bs{β} \) no considera cambios de variables ligadas entre términos, sin embargo, la noción de reducción \( \bs{αβ} = \bs{β}\cup\bs{α} \) si. De hecho, \( \convertible{\bs{αβ}} \) tiene las mismas propiedades que la convertibilidad \( =_{\bs{λ}} \) de la teoría formal \( \bs{λ} \).
...
\begin{defn}[Clasificación de términos]
  Dada una noción \( \bs{R} \), los términos lambda pueden ser clasificados con respecto a esta regla de la siguiente manera:
  \begin{itemize}
  ...
  \item Se dice que \( M \) tiene la \( R \)-forma normal \( N \) si \( M \convertible{R} N \).
  \end{itemize}
\end{defn}
...
Así como se definieron las nociones de reducción \( \bs{α} \) y \( \bs{β} \), se puede definir una noción de reducción para el axioma de extensionalidad de la teoría \( \bs{λη} \):

\[ \bs{η}\ :\ λx.M\, x \contract{} M \text{ dado que \( x\not\in \FV(M) \)} \]

Estudiar las transformaciones en \( \bs{α}\cup\bs{β}\cup\bs{η} \) es equivalente a estudiar la demostrabilidad en la teoría \( \bs{λη} \).
...
Sea \( M \) un término lambda, se puede categorizar su \( R \)-reducción a partir de \( G_{R}(M) \) con las siguientes definiciones
\begin{enumerate}
\item \( M \) \( R \)-normaliza fuertemente, denotado \( R\!-\!SN(M) \), si no hay una \( R \)-reducción infinita a partir de \( M \);
\item \( M \) es \( R \)-infinita, denotado \( R\!-\!\infty(M) \), si no \( R\!-SN\!(M) \);
\item \( R \) normaliza fuertemente, denotado \( SN \), si para toda \( M \), \( R\!-\!SN(M) \).
\end{enumerate}

Sea \( G_{β}(M) \) una \( β \)-gráfica de un término \( M \), es posible que \( M \) tenga una \( β \)-forma normal pero que \( G_{β}(M) \) sea infinita. Por ejemplo, si consideramos el término
...
Página 14 Seldin y Página 67 Barendregt

\subsection{Estrategias de reducción}
\label{sec:estrategias-reduccion}

Página 324 y comentarios de B. Pirce
